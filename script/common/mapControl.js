...
window.mapControl = {
    ...
    */
	...
	initDistrictOverlay: function() {
        ...
        //let params = {monitored_person:'#allentity'};
        let arrNum = Array.from({length:300}, (v,k) => k);
       /* var j=0;
        for (var i = 200; i < 400; i++) {
           arrNum[j++]=i;
        }*/
        let params = {fence_ids:arrNum.join(",")};

        //todo:从后台接口返回数据，将圆与多边形绘制上
        Urls.jsonp(Urls.listfence,params, function (data) {
            console.log(data);
          if (data.status === 0 && data.size > 0) {
                 data.fences.map(function (eitem, eindex) {
                    if(eitem.shape === "circle"){
                        let point = new BMap.Point(eitem.longitude, eitem.latitude);
                        let circle = new BMap.Circle(point,eitem.radius,styleOptions);
                        map.addOverlay(circle);//增加圆

                    } else if (eitem.shape === "polygon") {
                        let polygon = new BMap.Polygon(eitem.vertexes,styleOptions);
                        console.log(polygon)
                        map.addOverlay(polygon);//多边行
                    }
                });
            }
        });
        ...
        if (service_type === 1) {
            ...
            switch (status.substring(0, 2)) {
                ...
                case '静止':
                    ...
                    break;
                default:
                    iconUrl = __uri('/static/images/5.jpg');
                    break;
            }
        } else {
            ...
            switch (status.substring(0, 2)) {
                case '离线':
                    iconUrl = __uri('/static/images/feiji.png');
                    break;
                case '静止':
                    iconUrl = __uri('/static/images/caroff.png');
                    break;
                default:
                    iconUrl = __uri('/static/images/5.jpg');
                    break;
